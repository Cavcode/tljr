...
string sCageTag = "WolfDoor";   //tag of object creature should run too

void main()
{
     ActionSpeakString("damaged");
    object oSelf = OBJECT_SELF;
    int bTaskDone = GetLocalInt(oSelf,"gy_Task");
    if (bTaskDone<2)
    {
        object oCage = GetNearestObjectByTag(sCageTag);
        AssignCommand(oSelf,ClearAllActions(TRUE));
        AssignCommand(oSelf,ActionMoveToObject(oCage,TRUE));
        AssignCommand(oSelf,ActionOpenDoor(oCage));
        SetLocalInt(oSelf,"gy_Task",1);
        return;
    }
