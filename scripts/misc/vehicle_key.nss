     ...
     case X2_ITEM_EVENT_ACTIVATE:
         { // The item's CastSpell Activate or CastSpell UniquePower was just activated.
            ...
            // This is where you put your Unique power activation modifications.
                ...
                if (GetTag(oTarget) == "pug_speederbike")
                {
                    if (GetLocalString(oTarget, "BIKE_OWNER") == GetStringLeft(GetName(oItem),FindSubString(GetName(oItem),"'")))
                    {
                        int bLocked = GetLocalInt(oTarget, "SPEEDER_LOCKED");
                        if (bLocked)
                        {
                            SetLocalInt(oTarget, "SPEEDER_LOCKED", FALSE);
                            FloatingTextStringOnCreature("Vehicle Unlocked.", oItemUser, FALSE);
                            return;
                        }
                        else
                        {
                            SetLocalInt(oTarget, "SPEEDER_LOCKED", TRUE);
                            FloatingTextStringOnCreature("Vehicle Locked.", oItemUser, FALSE);
                            return;
                        }
                    }
                    else
                    {
                        FloatingTextStringOnCreature("Locked.", oItemUser, FALSE);
                        SendMessageToPC(oItemUser, "These are not the correct keys to this vehicle.");
                        return;
                    }
                }
                else
                {
                    SendMessageToPC(oItemUser, "You must target a vehicle with the keys.");
                    return;
                }
         }
         SetExecutedScriptReturnValue( X2_EXECUTE_SCRIPT_END);
