...
void AddIce(object oTarget, object oPC);
void MultipleAuraCheck(object oPC);
int HasIce(object oWeapon, object oPC);
...
void main()
{
  ...
  {
    if (!GetIsEnemy(oTarget, oPC)) AddIce(oTarget, oPC);
...
int HasIce(object oWeapon, object oPC)
{
  ...
  while(GetIsItemPropertyValid(ip))
  {
    if (GetItemPropertyDurationType(ip) == DURATION_TYPE_TEMPORARY)
    {
      ...
      {
        if (GetItemPropertySubType(ip) == IP_CONST_DAMAGETYPE_COLD) return TRUE;
      }
    }
...
void AddIce(object oTarget, object oPC)
{
  ...
  AddItemProperty(DURATION_TYPE_TEMPORARY, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_COLD, i), oWeapon, 30.0f);
}
