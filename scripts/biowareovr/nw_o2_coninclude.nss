...
// *
    ...
    {
//--NEW CHANGED MESSED UP BOOK CREATION FOR ZABEA HERE========================
//-- added by bloodsong (for searching)
//-- book sets:
//--    set 0 = common
//--    set 1 = uncommon
//--    set 2 = rare
...
//-- constants: max # of each type of book:

    int MAXBOOK1 = 60;
    int MAXBOOK2 = 15;
    int MAXBOOK3 = 4;

        int nPercent = Random(100)+1;
        string sRes = "0";
        int nMax = MAXBOOK1;

        if(nPercent <= 13)
        {  sRes = "2"; nMax = MAXBOOK3;  }
        if(nPercent < 43 && nPercent > 13)
        {  sRes = "1"; nMax = MAXBOOK2; }

        int nBook = Random(nMax)+1;
        if (nBook < 10)
        {  sRes = sRes+"book00" + IntToString(nBook);  }
        else if (nBook > 99)
        {  sRes = sRes+"book" + IntToString(nBook);  }
        else
        {  sRes = sRes+"book0" + IntToString(nBook);  }
