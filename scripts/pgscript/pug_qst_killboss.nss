...
void main()
{
    ...
    object oPC = GetFirstObjectInArea(GetArea(oKiller));

    //if (GetLocalInt(oKiller, "QUEST_BOSS_KILL") == FALSE)
        //{
        SetLocalIntOnAll(oKiller, "QUEST_BOSS_KILL", TRUE);
        //}

    /*while (GetIsObjectValid(oPC) == TRUE)
        {
        if (GetIsFriend(oPC, oKiller) == TRUE && GetIsPC(oPC) == TRUE && GetLocalInt(oPC, "QUEST_BOSS_KILL") == FALSE)
            {
            SetLocalInt(oPC, "QUEST_BOSS_KILL", TRUE);
            }
        oPC = GetNextObjectInArea(GetArea(oKiller));
        } */
