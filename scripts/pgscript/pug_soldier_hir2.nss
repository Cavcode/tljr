...
if(nHas < nMax && nGold >= COST)
    ...
    if (GetIsObjectValid(GetItemPossessedBy(oPC, "GalaticEmpireID"))==TRUE)
        {
        oHench = CreateObject(OBJECT_TYPE_CREATURE, "empiresoldier2", GetLocation(oPC), FALSE);
        }
    if (GetIsObjectValid(GetItemPossessedBy(oPC, "RebelAllianceID"))==TRUE)
        {
        oHench = CreateObject(OBJECT_TYPE_CREATURE, "rebelsoldier2", GetLocation(oPC), FALSE);
        }
