...
if(nHas < nMax && nGold >= COST)
    ...
    if (GetIsObjectValid(GetItemPossessedBy(oPC, "GalaticEmpireID"))==TRUE)
        {
        oHench = CreateObject(OBJECT_TYPE_CREATURE, "trademeetsold002", GetLocation(oPC), FALSE);
        }
    if (GetIsObjectValid(GetItemPossessedBy(oPC, "RebelAllianceID"))==TRUE)
        {
        oHench = CreateObject(OBJECT_TYPE_CREATURE, "rebelsoldier1", GetLocation(oPC), FALSE);
        }
