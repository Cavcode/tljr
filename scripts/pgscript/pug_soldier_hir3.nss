...
if(nHas < nMax && nGold >= COST)
    ...
    if (GetIsObjectValid(GetItemPossessedBy(oPC, "GalaticEmpireID"))==TRUE)
        {
        oHench = CreateObject(OBJECT_TYPE_CREATURE, "empiresoldier3", GetLocation(oPC), FALSE);
        }
    if (GetIsObjectValid(GetItemPossessedBy(oPC, "RebelAllianceID"))==TRUE)
        {
        oHench = CreateObject(OBJECT_TYPE_CREATURE, "rebelsoldier3", GetLocation(oPC), FALSE);
        }
