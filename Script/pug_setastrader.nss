...
else if (sSubject == "PROP")
    {
    while (sMember != GetCampaignString("cmrcgld", "CMRC_PROP_"+IntToString(x)) && x < 200)
        {
        x=x+1;
        }
    for (z = x; z < (nPropCount); z++)
        {
        SetCampaignString("cmrcgld", "CMRC_PROP_"+IntToString(z), GetCampaignString("cmrcgld", "CMRC_PROP_"+IntToString(z+1)) );
        }
    SetCampaignInt("cmrcgld", "CMRC_"+sMember, nRank+1);
    SetCampaignString("cmrcgld", "CMRC_PROP_"+IntToString(z+nPropCount), "");
...
while (GetIsObjectValid(oPlayer) == TRUE)
    {
    if (GetCampaignInt("cmrcgld", "CMRC_"+GetName(oPlayer))-1 != GetCampaignInt("cmrcgld", "CMRC_RANK", oPlayer) && GetIsObjectValid(GetItemPossessedBy(oPlayer, "TraderBadge"))==TRUE && GetCampaignInt("cmrcgld", "CMRC_"+GetName(oPlayer))>0 && GetCampaignInt("cmrcgld", "CMRC_RANK" ,oPlayer)>0)
        {
