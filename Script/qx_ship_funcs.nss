...
// Returns true if module debug on
int GetDebugMode();
...
// Returns TRUE if oObject is currently a ship
int GetIsShip(object oObject);

// Set oObject as being a ship or not
void SetIsShip(object oObject, int bIsShip);

//////////// IMPLEMENTATION /////////////////
int GetDebugMode()
{
    return GetLocalInt(GetModule(), DEBUG);
}

int GetIsShip(object oObject)
{
    int nIsShip = GetLocalInt(oObject, "PILOTING_SHIP");
    return nIsShip;
}

void SetIsShip(object oObject, int bIsShip)
{
    SetLocalInt(oObject, "PILOTING_SHIP", bIsShip);
}
...
void SetShipStatsOld(object oPC, int nShipType)
{
int nAC = GetAC(oPC);
...
eAB = EffectAttackIncrease(nAB+5, ATTACK_BONUS_MISC);
eAC = EffectACIncrease(nAC+5, AC_NATURAL_BONUS, AC_VS_DAMAGE_TYPE_ALL);
if (GetMaxHitPoints(oPC) < nHP)
    {
    eHP = EffectTemporaryHitpoints(nHP);
    }
effect eStr = EffectAbilityIncrease(ABILITY_STRENGTH, 20 - GetAbilityScore(oPC,ABILITY_STRENGTH,FALSE));
effect eDex = EffectAbilityIncrease(ABILITY_DEXTERITY, 20 - GetAbilityScore(oPC,ABILITY_DEXTERITY,FALSE));
...
NWNX_Creature_SetMovementRate(oPC, nSpeed);
