...
void main()
{
    ...
    if ((GetLastPerceptionVanished()) && GetIsEnemy(GetLastPerceived()))
    {
           ...
           GetAttackTarget() == GetLastPerceived()) && GetArea(GetLastPerceived()) != GetArea(OBJECT_SELF))
        {
           ...
           AssignCommand(OBJECT_SELF, ActionAttack(oPercep, FALSE));
        }
    }
    ...
    { // SpawnScriptDebugger();
        ...
        string sFaction;
        string sControl = GetCampaignString("starwars", GetTag(GetArea(OBJECT_SELF))+"_CONTROL");

        while (GetIsObjectValid(oPC)==TRUE)
            {
            if (GetStringLeft(GetTag(GetArea(oPC)), 7) == GetStringLeft(GetTag(GetArea(OBJECT_SELF)),7))
                {
                ...
                if (sFaction == sControl)
                    {
                    FloatingTextStringOnCreature(GetName(oPercep)+" has been spotted in "+GetName(GetArea(OBJECT_SELF)), oPC, FALSE);
                    }
                }
            }
